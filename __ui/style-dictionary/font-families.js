// config.js
module.exports = {
    source: [`tokens/font/font-basics.json`],
    ...
    format: {
      myFormat: ({dictionary, platform}) => {
        ...
        let mapName = 'font-families';
          
          let tokens = dictionary.allTokens.map(token => {

            if(String(token.name).startsWith("font-families")){
    ...
    platforms: {
       ...
       "scss": {
        ...
        "files": [{
          "destination": "font-families.scss", // nombre de archivo 
          "format": "myFormat", // formato 
          "mapName": "font-families"
        }]
      }
    }
  }